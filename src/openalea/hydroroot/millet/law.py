...
def compute_age_with_constant_growth_speed(g,sr_age=15,delta_lr_appareance=5,delta_cr_appareance=6):
    ...
    _orders = algo.orders(g,scale =g.max_scale())
    ramifs = [vid for vid in g.vertices(g.max_scale()) if g.edge_type(vid)=='+'] 
    age = {}
    sr= g.Trunk(g.root,g.max_scale())
    len_sr = len(sr)
    ...
    #             lr=list(lr)
    #             len_lr = len(lr)
    #             delta_lr = (sr_age) /float(len_lr)
    #             age[v] = max(age.get(cid,sr_age) for cid in g.children(v))-delta_lr
    ...
    #         if age_p +delta_lr_appareance < age_v:
    #             m = age_p + delta_lr_appareance
    #             for vid in local_axis(g,v):
    #                 age[v] = m
    #                 m= m+delta_lr

         

    # # Crown roots : they appear 6 days after germination
    ...
    #     date = date_cr_appareance
    #     cr = alog.local_axis(g, v)
    #     len_cr = len(list(cr))
    #     speed = (delta_cr_appareance-sr_age) / float(len_cr)
    #     for cid in cr:
    #         age[cid] = date_cr_appareance
    #         date += speed
