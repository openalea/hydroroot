...
def visitor_millet(g, v, turtle):
    ...
    if label == "collet":
        ...
        turtle.setColor(0)
        turtle.rollL(90.0)     # set reference frame for roots
        n.xyz = turtle.getPosition()
        return

    # ---------------------------------------------------------
    # Branching angles for '+' edges
    # ---------------------------------------------------------
    if g.edge_type(v) == '+':
        if label == "Crown":
            turtle.down(average_CR_angle)
        else:
            # deterministic without global seeding
            rng = random.Random(v)
            turtle.down(rng.normalvariate(53.52674, 15.14071))
