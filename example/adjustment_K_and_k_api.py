...
parser.add_argument("-d", "--data", help="csv file with the Jv(lcut) data to fit")
...
def K_k_adjustement(parameter, df_exp = None, Flag_Optim = False, output = None):
    """

    :param parameter: Parameter - (see :func: Parameters)
    :param df_exp: DataFrame (None) - data to fit
    :param Flag_Optim: boolean (False) - if True perform adjustment of k and K
    :param output: string (None) - if not None output filename
    :return: df: DataFrame with results

    df_exp: column names 'lcut' and 'J' with respectively the cut length in meter and the corresponding flux in microL/s
    ...
    archi_f = glob.glob(parameter.archi['input_dir'] + parameter.archi['input_file'])
    ...
    df = read_archi_data(archi_f) if parameter.archi['read_architecture'] else None
